...
- **完美绕过风控**：底层利用自定义音乐卡片结构，实测可稳定发送，不会被 QQ 屏蔽。
- **智能目标提取**：支持直接 `@某人`，插件会自动解析其昵称并写到卡片上；也支持纯文本指定名字。
- **平滑兼容缺省值**：如果不指定特定目标，默认对“你”发起关系邀请。
- **LLM 隔离**：深度优化事件机制，触发指令时自动阻断大语言模型（LLM），不会产生多余的对话回复。

## 📦 安装方法

### 方法一：通过 AstrBot 插件市场安装（推荐）
在 AstrBot 的控制台或管理面板中，搜索 `astrbot_plugin_fake_card` (替换为你实际发布的插件 ID)，点击安装并重启 AstrBot 即可。
